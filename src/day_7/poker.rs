...
impl Ord for Hand {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        ...
        if a == b {
            for item in self.cards.chars().zip(other.cards.chars()) {
                if item.0 != item.1 {
                    if item.0.is_ascii_digit() && item.1.is_ascii_digit() {
                        return item.1.cmp(&item.0);
                    } else if item.0.is_ascii_digit() {
                        return char_to_num(&item.1).cmp(&item.0.to_digit(10).unwrap());
                        // return std::cmp::Ordering::Greater;
                    } else if item.1.is_ascii_digit() {
...
impl Hand {
    fn get_type(&self) -> HandType {
        let mut char_map = self.cards.chars().fold(HashMap::new(), |mut acc, c| {
        ...
        let mut som: Vec<(char, u32)> = char_map.into_iter().collect();
