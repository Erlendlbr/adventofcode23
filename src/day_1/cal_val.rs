...
pub fn calc_inp() {
    ...
    let contents = fs::read_to_string(file2)
        .expect("Should have been able to read the file");
    
    let correctrd_accum: u32 = contents.split('\n')
    .map(|line| get_line_nr(tranform_line(line).as_str())).sum();
...
fn tranform_line(line: &str) -> String {
    let ret = line.replace("one", "one1one")
        .replace("two", "two2two")
